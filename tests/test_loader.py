...
class ConcreteLoader(DatabaseLoader):
    def connect(self, conn_str, dataset_config=None):
        pass

    def get_foreign_keys(self, table_name):
        pass

    def drop_foreign_keys(self, table_name, foreign_keys):
        pass

    def recreate_foreign_keys(self, table_name, foreign_keys):
        pass

    def cleanup(self):
        pass

    def get_last_successful_version(self, dataset):
        pass

    def update_metadata(self, version, dataset, success, row_count, error_message=None):
        pass

    def prepare_staging_schema(self, schema_name):
        pass

    def prepare_staging_table(self, table_name, schema):
        pass

    def bulk_load_native(self, table_name, parquet_uris, schema):
        pass

    def table_exists(self, table_name):
        pass

    def align_final_table_schema(self, staging_table, final_table):
        pass
    ...
    def recreate_indexes(self, indexes):
        pass

    def execute_merge_strategy(self, staging_table, final_table, primary_keys):
        pass

    def full_refresh_from_staging(self, staging_table, final_table, primary_keys):
        pass
...
def test_get_last_successful_version_raises_not_implemented(loader_instance):
    with pytest.raises(NotImplementedError):
        super(ConcreteLoader, loader_instance).get_last_successful_version(
            "dummy_dataset"
        )
...
def test_prepare_staging_table_raises_not_implemented(loader_instance):
    ...
    with pytest.raises(NotImplementedError):
        super(ConcreteLoader, loader_instance).prepare_staging_table(
            "dummy_table", schema
        )
...
def test_bulk_load_native_raises_not_implemented(loader_instance):
    schema = pa.schema([pa.field("foo", pa.int64())])
    with pytest.raises(NotImplementedError):
        super(ConcreteLoader, loader_instance).bulk_load_native(
            "dummy_table", [], schema
        )
...
def test_align_final_table_schema_raises_not_implemented(loader_instance):
    with pytest.raises(NotImplementedError):
        super(ConcreteLoader, loader_instance).align_final_table_schema(
            "staging", "final"
        )
...
def test_execute_merge_strategy_raises_not_implemented(loader_instance):
    with pytest.raises(NotImplementedError):
        super(ConcreteLoader, loader_instance).execute_merge_strategy(
            "staging", "final", ["id"]
        )
...
def test_full_refresh_from_staging_raises_not_implemented(loader_instance):
    with pytest.raises(NotImplementedError):
        super(ConcreteLoader, loader_instance).full_refresh_from_staging(
            "staging", "final", ["id"]
        )
