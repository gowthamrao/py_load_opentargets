...
def mock_fsspec():
    ...
    with patch(
        "py_load_opentargets.data_acquisition.fsspec.core.url_to_fs"
    ) as mock_url_to_fs:
        mock_fs = MagicMock()
...
def test_download_dataset_success(mock_url_to_fs, tmp_path: Path):
    ...
    with patch(
        "py_load_opentargets.data_acquisition._verify_file_checksum"
    ) as mock_verify:
    ...
    expected_calls = [
        call(
            "gcs://open-targets/platform/22.06/output/etl/parquet/targets/file1.parquet",
            str(expected_local_path / "file1.parquet"),
        ),
        call(
            "gcs://open-targets/platform/22.06/output/etl/parquet/targets/file2.parquet",
            str(expected_local_path / "file2.parquet"),
        ),
    ]
...
def test_get_remote_dataset_urls_gcs():
    ...
    urls = get_remote_dataset_urls(
        "gs://bucket/{version}/{dataset_name}", "22.06", "targets"
    )
    assert urls == []
