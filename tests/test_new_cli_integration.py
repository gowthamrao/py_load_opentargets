...
def cleanup_db(db_conn):
    ...
    with db_conn.cursor() as cursor:
        ...
        cursor.execute(
            "DROP TABLE IF EXISTS public.test_data_one, public.test_data_two, public.test_data_flat;"
        )
...
def mock_data_acquisition(monkeypatch, tmp_path: Path):
    ...
    dummy_data_two = pa.Table.from_pydict(
        {"key1": [10, 20], "key2": [30, 40], "value": ["x", "y"]}
    )
    ...
    flat_data = pa.Table.from_pydict(
        {
            ...
            "other_nested": [{"info": "foo"}, {"info": "bar"}],
        }
    )
    ...
    monkeypatch.setattr(
        "py_load_opentargets.cli.list_available_versions", mock_list_versions
    )
    monkeypatch.setattr(
        "py_load_opentargets.validator.list_available_versions", mock_list_versions
    )
    monkeypatch.setattr(
        "py_load_opentargets.api.list_available_versions", mock_list_versions
    )
    # These two are called from the orchestrator
    monkeypatch.setattr(
        "py_load_opentargets.orchestrator.download_dataset", mock_download
    )
    monkeypatch.setattr(
        "py_load_opentargets.orchestrator.get_checksum_manifest",
        mock_get_checksum_manifest,
    )
...
def test_new_cli_end_to_end(
    db_conn, db_conn_str, mock_data_acquisition, test_config, mock_plain_logging
):
    """
    ...
    result = runner.invoke(
        cli, args, catch_exceptions=False, env={"DB_CONN_STR": db_conn_str}
    )
    ...
    with psycopg.connect(db_conn_str) as verify_conn:
        with verify_conn.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT key1, key2, value FROM public.test_data_two ORDER BY key1;"
            )
            ...
            cursor.execute(
                "SELECT dataset_name, status FROM _ot_load_metadata ORDER BY dataset_name;"
            )
            meta_data = cursor.fetchall()
            assert meta_data == [
                ("test_data_one", "success"),
                ("test_data_two", "success"),
            ]


def test_cli_flattening(
    db_conn, db_conn_str, mock_data_acquisition, test_config_flatten, mock_plain_logging
):
    """
    ...
    cursor.execute(
        "SELECT id, location_chromosome, location_start, other_nested FROM public.test_data_flat ORDER BY id;"
    )
    ...
    parsed_data = [
        (
            ...
            loc_chrom,
            loc_start,
            json.loads(other_nested) if isinstance(other_nested, str) else other_nested,
        )
...
def test_cli_full_refresh(
    ...
    cursor.execute(
        "INSERT INTO public.test_data_one (id, data) VALUES (-1, 'old_data');"
    )
    db_conn.commit()
...
def test_validate_command_success(monkeypatch, test_config, db_conn_str):
    ...
    monkeypatch.setattr(
        "py_load_opentargets.cli.ValidationService", mock_validator_class
    )
...
def test_load_command_fails_on_invalid_config(
    ...
    monkeypatch.setattr(
        "py_load_opentargets.orchestrator.ETLOrchestrator.run", mock_orchestrator_run
    )

    runner = CliRunner()
    args = [
        "--config",
        str(test_config_invalid),
        "load",
        "test_data_one",
    ]
    ...
    result = runner.invoke(
        cli, args, catch_exceptions=True, env={"DB_CONN_STR": db_conn_str}
    )
...
def test_cli_json_logging_from_config(
    db_conn_str, mock_data_acquisition, test_config_json_log
):
    """
    ...
    try:
        ...
        pytest.fail(
            "The first line of output was not a valid JSON string, even when configured via file."
        )
