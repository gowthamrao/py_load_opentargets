...
""")
    ...
    assert (
        config["source"]["data_download_uri_template"]
        == "s3://my-bucket/{dataset_name}/"
    )
    assert (
        config["source"]["checksum_uri_template"]
        == "s3://my-bucket/checksums/{version}"
    )
