    ...
    original_schema = pa.schema(
        [
            ...
            pa.field(
                "data_to_flatten",
                pa.struct(
                    [pa.field("field_a", pa.string()), pa.field("field_b", pa.int32())]
                ),
            ),
...
def test_generate_create_table_sql_simple(loader):
    ...
    sql = loader._generate_create_table_sql(
        "public.my_table", schema, schema_overrides={}
    )
...
def test_generate_create_table_sql_with_flattened_schema(loader):
    ...
    sql = loader._generate_create_table_sql(
        "staging.flat_table", flattened_schema, schema_overrides={}
    )
...
def test_postgres_loader_adheres_to_abc():
    ...
    try:
        ...
        pytest.fail(
            f"PostgresLoader failed to instantiate. It might be missing implementation for an abstract method. Error: {e}"
        )
...
def test_align_final_table_schema_detects_all_drift_types(loader, mocker, caplog):
    ...
    mocker.patch.object(
        loader, "_get_table_schema_from_db", side_effect=[staging_schema, final_schema]
    )
    ...
    assert (
        "Staging table type is 'text', but final table type is 'double precision'"
        in caplog.text
    )
    assert (
        "exist in the final table 'final.table' but not in the new data source: old_column"
        in caplog.text
    )
