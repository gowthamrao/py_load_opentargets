...
def mock_s3_data(s3_client):
    ...
    dummy_table = pa.Table.from_pydict({"id": ["target1", "target2"]}, schema=dummy_schema)
    ...
    s3_client.put_object(Bucket=TEST_BUCKET, Key=manifest_checksum_key, Body=f"{manifest_hash}".encode('utf-8'))
...
def mock_db_loader_factory():
    ...
    with patch('py_load_opentargets.orchestrator.get_db_loader_factory') as mock_get_factory:
...
def test_s3_orchestration_e2e(mock_get_checksum, mock_verify_remote, mock_get_urls, mock_get_schema, s3_client, mock_s3_data, s3_test_config, mock_db_loader_factory):
    """
    ...
    mock_get_checksum.return_value = {"output/etl/parquet/targets/part-0.parquet": "dummy_hash"}
    mock_get_urls.return_value = [f"s3://{TEST_BUCKET}/platform/{TEST_VERSION}/output/etl/parquet/targets/part-0.parquet"]
    ...
    assert loaded_uris[0].startswith(f"s3://{TEST_BUCKET}/platform/{TEST_VERSION}/output/etl/parquet/targets/part-0.parquet")
    ...
    assert metadata_kwargs['row_count'] == 2 # From the dummy parquet file and mock return
