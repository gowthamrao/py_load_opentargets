...
def test_ftp_data_download_and_load(db_conn, db_conn_str):
    ...
    try:
        ...
        cursor.execute("SELECT dataset_name, status FROM _ot_load_metadata WHERE opentargets_version='22.04' AND dataset_name='drugwarnings';")
        ...
        with db_conn.cursor() as cursor:
            ...
            cursor.execute('DROP TABLE IF EXISTS public.drugwarnings, staging.drugwarnings;')
            db_conn.commit()
...
def test_empty_parquet_file(db_conn, db_conn_str, tmp_path: Path):
    ...
    try:
        ...
        # Check metadata table
        cursor.execute("SELECT dataset_name, status FROM _ot_load_metadata WHERE opentargets_version='22.04' AND dataset_name='empty_dataset';")
        meta_result = cursor.fetchone()
        ...
        with db_conn.cursor() as cursor:
            cursor.execute("TRUNCATE _ot_load_metadata;")
            cursor.execute('DROP TABLE IF EXISTS public.empty_dataset, staging.empty_dataset;')
            db_conn.commit()
...
def test_schema_mismatch(db_conn, db_conn_str, tmp_path: Path):
    ...
    mock_config = {
        ...
        'datasets': {
            'mismatch_dataset': {'primary_key': ['id']}
        }
    }
    ...
    with pytest.raises(polars.exceptions.ColumnNotFoundError), \
         patch('py_load_opentargets.orchestrator.get_remote_dataset_urls') as mock_get_urls, \
         patch('py_load_opentargets.orchestrator.get_checksum_manifest') as mock_get_checksums, \
         patch('py_load_opentargets.orchestrator.verify_remote_dataset') as mock_verify, \
         patch('os.getenv') as mock_getenv:
...
def test_data_integrity_error(db_conn, db_conn_str, tmp_path: Path):
    ...
    mock_config = {
        ...
        'datasets': {
            'integrity_dataset': {'primary_key': ['id']}
        }
    }
