...
def test_download_integration_run(db_conn, db_conn_str, tmp_path: Path):
    ...
    targets_data = pa.Table.from_pydict(
        {"id": ["target1", "target2"], "value": [100, 200]}
    )
    diseases_data = pa.Table.from_pydict(
        {"id": ["diseaseA", "diseaseB"], "description": ["descA", "descB"]}
    )
    ...
    mock_config = {
        ...
        "source": {
            ...
            "local": {
                ...
                "data_download_uri_template": f"file://{version_path}"
                + "/{dataset_name}/",
    ...
    try:
        ...
        cursor.execute(
            "SELECT dataset_name, status FROM _ot_load_metadata WHERE opentargets_version='22.04' ORDER BY dataset_name;"
        )
        ...
        with db_conn.cursor() as cursor:
            ...
            cursor.execute(
                "DROP TABLE IF EXISTS public.targets, public.diseases, staging.targets, staging.diseases;"
            )
            db_conn.commit()
